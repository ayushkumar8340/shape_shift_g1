...
class G1CrouchEnv(BaseEnv):
    def __init__(self, cfg, headless):
        super().__init__(cfg, headless)

    def _create_command_generator(self):
        base_height = self.cfg.commands.ranges.base_height
        if base_height is None:
            raise RuntimeError("Crouch env requires cfg.commands.ranges.base_height=(min_h,max_h).")
